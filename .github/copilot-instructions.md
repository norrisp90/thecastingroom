...
- **Azure OpenAI Realtime API — GA vs Legacy protocol**: The GA protocol (`/openai/v1/realtime/client_secrets`) requires **Entra ID (AAD) auth** — does NOT work with api-key. Use the **legacy protocol** (`/openai/realtimeapi/sessions?api-version=2025-04-01-preview`) which supports api-key auth. The WebRTC SDP exchange goes to a **regional endpoint** (`https://swedencentral.realtimeapi-preview.ai.azure.com/v1/realtimertc`) that's publicly accessible (no VNet restriction). Current realtime model: `gpt-realtime-mini`.
