...
- **Fastify addHook vs route preHandler**: Plugin-level `fastify.addHook("preHandler", fn)` CANNOT be bypassed by setting `preHandler: []` on a route — it always runs. To skip auth for WebSocket routes, check `request.headers.upgrade === "websocket"` inside the hook and return early. The WS route handles auth manually via `?token=` query param.
- **Azure OpenAI Realtime API — WebSocket relay**: Voice auditions use a WebSocket relay through the backend. Browser connects to `wss://backend/api/worlds/:id/auditions/:id/realtime-ws?token=JWT`, backend opens a WS to `wss://oai-*.openai.azure.com/openai/realtime?api-version=2025-04-01-preview&deployment=gpt-realtime-mini` (preview endpoint) with `api-key` header, and relays messages bidirectionally. **Classic `kind:"OpenAI"` resources require the preview format** (`/openai/realtime?api-version=...&deployment=...`). The GA format (`/openai/v1/realtime?model=...`) does NOT work on classic resources — returns 400. Browser captures mic audio via AudioWorklet → PCM16 24kHz mono base64 → Realtime API `input_audio_buffer.append` events; receives `response.audio.delta` base64 PCM16 → decodes and plays via AudioContext. Current realtime model: `gpt-realtime-mini`.
