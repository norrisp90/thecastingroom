...
- **Azure OpenAI Realtime API — WebSocket relay**: Voice auditions use a WebSocket relay through the backend. Browser connects to `wss://backend/api/worlds/:id/auditions/:id/realtime-ws?token=JWT`, backend opens a WS to `wss://oai-*.openai.azure.com/openai/v1/realtime?model=gpt-realtime-mini` (GA endpoint) with `api-key` header, and relays messages bidirectionally. **Do NOT use the deprecated preview format** (`/openai/realtime?api-version=...&deployment=...`). Browser captures mic audio via AudioWorklet → PCM16 24kHz mono base64 → Realtime API `input_audio_buffer.append` events; receives `response.audio.delta` base64 PCM16 → decodes and plays via AudioContext. Current realtime model: `gpt-realtime-mini`.
